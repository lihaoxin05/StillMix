...
framename_format = 'frame{:06d}.jpg'
# framename_format = '{:05d}.jpg'

class VideoRecord(object):
    def __init__(self, row):
        self._data = row
        self.path =  self._data[0]
        self.num_frame = int(self._data[1])
        self.label = int(self._data[2])


class Dataset(torch.utils.data.Dataset):
    def __init__(self, data_root_path, list_file, transform=None, mode='video', number_of_frame=1, frame_skip=1):
    ...
    def _load_image(self, image_path):
        try:
            return Image.open(os.path.join(self.data_root_path, image_path)).convert('RGB')
        except Exception:
            print('error loading image:', os.path.join(self.data_root_path, image_path))
            assert False
    ...
    def _parse_list(self):
        ...
        if self.mode == 'video':
            ...
            for item in tmp:
                ...
                for ii in range(0, num_frame, self.frame_skip):
                    record_c = copy.deepcopy(record)
                    record_c.path = os.path.join(record_c.path, framename_format.format(ii+1))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'video':
            if self.number_of_frame == 1:
                frame_index = np.random.randint(1, record.num_frame+1)
                frame_path = os.path.join(record.path, framename_format.format(frame_index))
            else:
                frame_index = np.random.randint(1, record.num_frame+1, self.number_of_frame)
                frame_path = [os.path.join(record.path, framename_format.format(ind)) for ind in frame_index]
        elif self.mode == 'frame':
            frame_path = record.path
        if self.number_of_frame == 1:
            full_path = os.path.join(self.data_root_path, frame_path)
            if not os.path.exists(full_path):
                print('################## Not Found:', full_path)
                assert False
            image = self._load_image(frame_path)
            process_data = self.transform(image)
        else:
            image = []
            for fp in frame_path:
                full_path = os.path.join(self.data_root_path, fp)
                if not os.path.exists(full_path):
                    print('################## Not Found:', full_path)
                    assert False
                image.append(self.transform(self._load_image(full_path)))
            process_data = torch.stack(image, dim=0)
            frame_path = record.path
        
        return frame_path, process_data, record.label
...
class Model(nn.Module):
    ...
    def prepare_model(self):
        if self.pretrain:
            ...
            weights = 'DEFAULT'
        self.base_model = getattr(models, self.arch)(weights=weights)
...
def load_split_train_test(datadir, train_list, test_list, batch_size):
    train_transforms = transforms.Compose([transforms.Resize((232,232)),
    ...
    testloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, num_workers=4, shuffle=False)
...
def main(args):
    ...
    if args.test:
        ...
        test_data = Dataset(args.data_dir, args.test_list, transform=test_transforms, mode=args.test_mode, number_of_frame=args.video_mode_frame, frame_skip=args.frame_skip)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frame_skip', type=int, default=1, help='')
