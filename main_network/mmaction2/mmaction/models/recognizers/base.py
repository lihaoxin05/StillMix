...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        ### test blending, e.g., select one frame, shuffle
        self.test_blending = None
        if test_cfg is not None and 'blending' in test_cfg:
            from mmcv.utils import build_from_cfg

            from mmaction.datasets.builder import BLENDINGS
            self.test_blending = build_from_cfg(test_cfg['blending'], BLENDINGS)
    ...
    def forward(self, imgs, label=None, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            if self.blending is not None:
                imgs, label = self.blending(imgs, label, **kwargs)
            return self.forward_train(imgs, label, **kwargs)
        ### test blending
        if self.test_blending is not None:
            imgs, label = self.test_blending(imgs, label, **kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        if 'human_mask' in data_batch.keys():
            human_mask = data_batch['human_mask']
            aux_info['human_mask'] = human_mask
